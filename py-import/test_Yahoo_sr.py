...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate SpectralResidualAnomalyDetector on Yahoo S5 in IoTDB.")
    ...
    parser.add_argument("--benchmarks", default="all", help="Benchmarks to test, e.g. all or A1,A2,A3,A4.")
    parser.add_argument("--files", default="all", help="CSV file stems/names to test, e.g. all or real_1,A4Benchmark-TS1.")
    parser.add_argument("--window-size", type=int, default=100)
    parser.add_argument("--threshold", type=float, default=3.0)
    parser.add_argument("--amp-window-size", type=int, default=5)
    parser.add_argument("--score-window-size", type=int, default=50)
...
def build_sql(args, device):
    return (
        ...
        f"'amp_window_size'='{args.amp_window_size}', "
        f"'score_window_size'='{args.score_window_size}') "
        f"FROM {device}"
    )
...
def run(session, args, yahoo_files):
    ...
    print(
        ...
        f"threshold: {args.threshold}\namp_window_size: {args.amp_window_size}\n"
        f"score_window_size: {args.score_window_size}\ntolerance: {args.tolerance}"
    )

    print("\n========== Evaluating Yahoo S5 Files ==========")
    for index, (benchmark_key, csv_path) in enumerate(yahoo_files, start=1):
        ...
        if index % 50 == 0:
            print(f"processed {index}/{len(yahoo_files)}")
