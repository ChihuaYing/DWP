...
使用 `test_Yahoo_baseline.py` 在 Yahoo S5 全量 367 条序列上测试 IoTDB 内置异常检测 UDF，标签按点级别精确匹配计算 `precision / recall / F1`。LOF 因查询超时未纳入；MISSDETECT 和 RANGE 在该数据集/调用方式下没有有效结果，后续不作为主要对比。

| 算法 | Precision | Recall | F1 | 检测点数 | 备注 |
|---|---:|---:|---:|---:|---|
| IQR | 0.208873 | 0.359777 | 0.264302 | 7100 | 相对均衡，但召回有限 |
| KSIGMA | 0.596415 | 0.234110 | 0.336237 | 1618 | Precision 最高，误报少，但漏检较多 |
| TWOSIDEDFILTER | 0.006009 | 0.448569 | 0.011859 | 307703 | 输出过多，FP 很高 |
| OUTLIER | 0.006738 | 0.872149 | 0.013374 | 533506 | Recall 高，但几乎全靠大量报警换来 |

当前这组 baseline 里，`KSIGMA` 的整体 F1 最好，主要原因是误报数量明显少于其他方法；`IQR` 次之。`TWOSIDEDFILTER` 和 `OUTLIER` 在 Yahoo S5 上报警量过大，点级 F1 很低。
